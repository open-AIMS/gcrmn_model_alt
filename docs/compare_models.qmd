...
\begin{align}
  ...
  logit(\mu) &= \mathbf{\beta_y} \mathbf{X_y} + \mathbf{\gamma_d} Z_d + \omega\mathbf{\gamma_t} Z_t +
    \mathbf{\gamma_s} Z_s + \mathbf{\gamma_r} Z_r\\
  \mathbf{\gamma_d} &= \sigma_d\times z_d\\
  \mathbf{\gamma_t} &= \sigma_t\times z_t\\
  \mathbf{\gamma_s} &= \sigma_s\times z_s\\
  \mathbf{\gamma_r} &= \sigma_r\times z_r\\
  \phi &\sim \Gamma(0.01,0.01)\\
  z_d,z_t,z_s,z_r &\sim N(0,1)\\
  \sigma_d,\sigma_t,\sigma_s,\sigma_r &\sim t(3, 0, 1)\\
  \sigma_y &\sim t(3, 0, 0.5)\\ 
  \beta_{y} &\sim N(\mathbf{y}, \sigma_y)\\ 
...
and cell-means Year model matrices respectively, $\mathbf{\gamma_d}$,
$\mathbf{\gamma_g}$, $\mathbf{\gamma_s}$ and $\mathbf{\gamma_r}$ are
the sum-to-zero varying effects, and $Z_d$, $Z_g$, $Z_s$ and $Z_r$
represent the Dataset_id, Grid_id, Site and Transect/Replicate codes
respectively. $\mathbf{y}$ represents year hyperparameters and $y$ is a year
iterator. $I_y$ and $O_y$ represent the initial observed year and mean
observed response within the MEOW Ecoregion.  $F_{y}$ and $G_{y}$
represent final observed year and gap years within the MEOW Ecoregion.
